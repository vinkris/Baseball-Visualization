...
class vizAPI
{
    ...
    function getMajorLeaguePlayersCountByState($state)
    {
        $result = array();
        $result = $this->objmysql->escapeString(array($state));
        $state = $result[0];
        $query = "select birthState, count(*) as count from master where playerID in (select playerID from allstarfull where lgID = 'AL' or lgID = 'NL') and birthState = '$state'";
        $result = $this->objmysql->getArrayHash($query);
        return $result;
    }
    ...
    {
        $result = array();
        $query = "select distinct schoolState from schools";
        $result = $this->objmysql->getArrayHash($query);
        return $result;
    }
